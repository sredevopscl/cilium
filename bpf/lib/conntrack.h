...
ct_entry_expired_for_rebalance(const struct ct_entry *entry, bool is_tcp)
{
	__u32 wait_time = is_tcp ?
			  bpf_sec_to_mono(CT_SERVICE_CLOSE_REBALANCE) :
			  bpf_sec_to_mono(CT_SERVICE_LIFETIME_NONTCP);
					...
					__u32 *monitor)
{
	...
	if (entry) {
		...
		if (dir == CT_SERVICE &&
		    ((is_tcp && ct_entry_closing(entry) &&
		      (seen_flags.value & TCP_FLAG_SYN)) ||
		     !is_tcp) &&
		    ct_entry_expired_for_rebalance(entry, is_tcp))
			goto ct_new;
#endif
